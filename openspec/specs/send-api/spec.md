...
The Unix-socket HTTP surface of allowmaild: the `POST /v1/send` request/response contract, strict input validation, `GET /v1/health`, and sanitized errors. Every guarantee holds when the socket is called directly, without any client-side plugin.
